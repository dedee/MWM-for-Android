...
public class GoogleWeatherEngine extends AbstractWeatherEngine {
	...
	public WeatherData update(Context context,
		...
		if (isGeolocationDataUsed()) {
			...
			queryString = "http://www.google.com/ig/api?weather=,,," + lat
					+ "," + lon;
		} else {
		...
		if (rp.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
			...
			if (ws == null || ws.getWeatherCurrentCondition() == null) {
				...
				for (int i = 0; i < days; ++i) {
					...
					weatherData.forecast[i].setIcon(getIcon(wfc.getCondition()
							.toLowerCase()));
					...
					if (Preferences.weatherCelsius) {
						...
						weatherData.forecast[i].setTempHigh(WeatherUtils
								.celsiusToFahrenheit(wfc.getTempMaxCelsius()));
						weatherData.forecast[i].setTempLow(WeatherUtils
								.celsiusToFahrenheit(wfc.getTempMinCelsius()));
					}
				...
				if (Preferences.weatherCelsius) {
					...
					weatherData.temp = Integer
							.toString(wcc.getTempFahrenheit());
				}
